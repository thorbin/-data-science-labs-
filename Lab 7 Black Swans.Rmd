...
#2006 plot# 
ggplot(data=power_separated_month_2006,mapping=aes(x=Minutes,y=mean_RP))+geom_point()+geom_smooth()+facet_wrap(~ Month,nrow=NULL)
```
```{r}
#2007#
...
#plot#
ggplot(data=power_separated_month_2007,mapping=aes(x=Minutes,y=mean_RP))+geom_smooth()+facet_wrap(~ Month, nrow=NULL)
```

#### 2007 plot comments: 
It appears as though nearly all months of the year share a similarity in terms of two local maxima (not including end points). Global reactive power was chosen as the variable because the quantity power tells us how much current changes, and sudden and periodic changes in current would indicate a human presence in a home. We can see that generally in the winter months power consumption is actually fairly low in magnitude where as it is significantly larger in the warmer months. This is likely due to the fact that this home used gas heating in the colder months and an electronic AC unit in the warmer months. This explanation appears to hold when looking at months after september ie, when the weather starts getting cooler, which is  seen by a lower mean power usage in the months of 10-12. It is also interesting to observe two glaring "2-gallon hat" maxima in nearly all plots. These could be the time a person dwelling gets home perhaps one peak is when the person takes a shower requiring the electric water heater to supply more and more power to deal with the sudden demand for warm water. As for the second peak
```{r}
#2008#
...
#plot#
ggplot(data=power_separated_month_2008,mapping=aes(x=Minutes,y=mean_RP))+geom_smooth()+facet_wrap(~Month, nrow=NULL)
```
...
#plot#
ggplot(data=power_separated_month_2009,mapping=aes(x=Minutes,y=mean_RP))+geom_smooth()+facet_wrap(~Month, nrow=NULL)
```

```{r}
#2010#
power_separated_month_2010<-filter(power_separated,Year==2010) %>% mutate(Minutes=(Hour%%100)*60+Minute)%>%group_by(Month,Minutes)%>%mutate(mean_Voltage=mean(Voltage)/10^6,mean_GI=mean(Global_intensity),mean_RP=mean(Global_reactive_power)) %>% ungroup(Minutes)
#plot#
ggplot(data=power_separated_month_2010,mapping=aes(x=Minutes,y=mean_RP))+geom_smooth()+facet_wrap(~Month, nrow=NULL)
```
