...
```
### Thorbin's question: How does seasonal weather (month) affect the  of flights? 
```{r}
#code chunk#
...
Thorbin's Conclusion:
  ...
  For 2017 it appears as though weather delays and the month of year have a confounding relationship. My analysis confirmed a well documented observation that the weather tends to be more severely incliment in the winter months which certainly had an effect on the weather delays at DIA for 2017. 
  
# Jorge's Analysis 
Question: How do individual months affect arriving flights (destination is DIA) in terms of being delayed and early? How does distance affect arriving flights (destination is DIA) in term of being delayed and early?  
...
###Delayed, flights that arrive 15 minutes or more from scheduled time. On-time means any flight arriving early or within 14 minutes 
```{r  echo=FALSE}
...
```
### The Percentage of flights that arrived late: 15.65756 %
```{r}
...
```
Percentage of flights that where late in JAN in all the flights that where late
We can see that late flights in JAN make a significant percentage of all fights that where late (10%)
```{r}
...
```
Percentage of flights that where late in MAY in all the flights that where late. 
...
```
Percentage of flights that where late in JUN in all the flight that where late 
...
```
Percentage of flights that where late in AUG in all the flight that were late. 
...
```
Percentage of flights that where late in DEC in all the flight that where late.
...
```
Percentage of flights that where late in FEB in all the flight that where late. 
...
```
Percentage of flights that where late in March in all the flight that where late. 
...
```
percentage of flights that where late in September in all the flight that where late
...
```
percentage of flights that where late in October in all the flight that where late 
...
```
percentage of flights that where late in november in all the flight that where late 
...
```
Percentage of flights that where late in April in all the flight that where late 
...
# Mean percentage of month that has less chance of flights arriving late 
```{r}
...
could there be an intercation somewhere between 895.3668 and 922.6956 miles that may cause flight to be delayed on avg.
